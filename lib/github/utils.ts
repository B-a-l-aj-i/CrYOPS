/**
 * Pure utility functions for GitHub data processing
 * These functions are generic and not GitHub-specific
 */
 ...
 */
export const yearOverYearChangePercentage = (
  ...
  const last12MonthsContributions = contributions.filter(
    (contrib: { date: string }) => {
  ...
  const prev12MonthsContributions = contributions.filter(
    (contrib: { date: string }) => {
      const contribDate = new Date(contrib.date);
  ...
  return Math.round(
    ((last12MonthsTotal - prev12MonthsTotal) / prev12MonthsTotal) * 100
  );
};
 ...
 */
export const quarterOverQuarterChangePercentage = (
  contributions: { date: string; count: number }[]
) => {
  const now = new Date();
  now.setHours(23, 59, 59, 999); // End of today
  ...
  const last3MonthsContributions = contributions.filter(
    (contrib: { date: string }) => {
      const contribDate = new Date(contrib.date);
  ...
  const prev3MonthsContributions = contributions.filter(
    (contrib: { date: string }) => {
      const contribDate = new Date(contrib.date);
  ...
  return Math.round(
    ((last3MonthsTotal - prev3MonthsTotal) / prev3MonthsTotal) * 100
  );
};
 ...
 */
export const halfOverHalfChangePercentage = (
  contributions: { date: string; count: number }[]
) => {
  const now = new Date();
  now.setHours(23, 59, 59, 999); // End of today
  ...
  const last6MonthsContributions = contributions.filter(
    (contrib: { date: string }) => {
      const contribDate = new Date(contrib.date);
  ...
  const prev6MonthsContributions = contributions.filter(
    (contrib: { date: string }) => {
      const contribDate = new Date(contrib.date);
  ...
  return Math.round(
    ((last6MonthsTotal - prev6MonthsTotal) / prev6MonthsTotal) * 100
  );
};
...
export const calculatePeriodMetrics = (
  ...
  const weekendPercentage =
    periodTotal > 0 ? Math.round((weekendTotal / periodTotal) * 100) : 0;
  const weekdayPercentage =
    periodTotal > 0 ? Math.round((weekdayTotal / periodTotal) * 100) : 0;

  // Find most active day of week
  const mostActiveDay =
    Object.entries(dayOfWeekCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || null;

  // Calculate average daily commits
  const daysWithCommits = filteredContributions.filter(
 ...
 */
export function getActiveYears(contributionsData: {
 ...
 */
export function getFirstCommitInfo(
  contributions: Array<{ date: string; count: number }>
  ...
  const yearsSince =
    (new Date().getTime() - firstDate.getTime()) / (1000 * 60 * 60 * 24 * 365);
...
export function getTotalContributions(
  ...
  return contributions.reduce((sum, contrib) => sum + contrib.count, 0);
}

/**
 * Sum contributions for current year
 */
export function getCurrentYearContributions(
  contributions: Array<{ date: string; count: number }>
): number {
