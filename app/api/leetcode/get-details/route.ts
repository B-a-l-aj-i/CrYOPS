  ...
  try {
    ...
    try {
      ...
      const acceptedSubmissions =
        (profileData.recentSubmissions as Submission[])?.filter(
          (sub) => sub.statusDisplay === "Accepted"
        ) || [];
      const randomProblem =
        acceptedSubmissions.length > 0
          ? acceptedSubmissions[
              Math.floor(Math.random() * acceptedSubmissions.length)
            ]
          : null;

      // Calculate acceptance rate
      const totalSubmissions =
        profileData.totalSubmissions?.[0]?.submissions || 0;
      ...
      if (Object.keys(submissionCalendar).length > 0) {
        ...
        const sortedEntries = calendarEntries.sort((a, b) => {
          const timestampA =
            typeof a[0] === "string" ? parseInt(a[0]) : (a[0] as number);
          const timestampB =
            typeof b[0] === "string" ? parseInt(b[0]) : (b[0] as number);
        ...
        for (let i = 0; i < sortedEntries.length; i++) {
          ...
          if ((count as number) > 0) {
            if (i === 0) {
              ...
              const prevTimestamp =
                typeof prevTimestampKey === "string"
              ...
              const daysDiff = Math.floor(
                (timestampNum - prevTimestamp) / oneDay
              );
